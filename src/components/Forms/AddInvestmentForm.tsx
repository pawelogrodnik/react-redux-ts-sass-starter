                    ...
                    <div className="form--addInvestment_bottom">
                        ...
                        <div className="form--addInvestment_r">
                            <div className="form--addInvestment__content form--addInvestment__content--blankTitle">
                                {(investmentType === parcel || investmentType === flat || investmentType === hotel || investmentType === dorm) && <Field name="detailedParams.usableArea" component={RegularField} label={'Powierzchnia [m2]'} placeholder={'Powierzchnia [m2]'} validate={[V.required]} />}
                                {investmentType === parcel && <Field name="detailedParams.pum" component={RegularField} label={'PUM [m2]'} placeholder={'PUM [m2]'} validate={[V.required]} />}
                                {investmentType === flat && <Field name="detailedParams.floorNumber" component={RegularField} label={'Piętro'} placeholder={'Piętro'} validate={[V.required]} />}
                                <Field name="detailedParams.priceService" component={RegularField} label={'Cena w serwisie'} placeholder={'Cena w serwisie'} validate={[V.required]} />
                                {(investmentType === flat || investmentType === parcel || investmentType === hotel || investmentType === dorm || investmentType === vehicle) && <Field name="detailedParams.priceMarket" component={RegularField} label={'Cena rynkowa'} placeholder={'Cena rynkowa'} validate={[V.required]} />}
                                {investmentType === parcel && <Field name="detailedParams.roi" component={RegularField} label={'ROI'} placeholder={'ROI'} validate={[V.required]} />}
